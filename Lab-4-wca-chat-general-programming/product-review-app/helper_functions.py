...
def filter_similar_words(listA):
    
    # To filter out similar or plural forms of words in a list
    
    filtered_list = []
    for word in listA:
        if any(w for w in listA if w != word and word in w):
            continue
...
def count_datapoints_in_review(review):
    # List of datapoints to check for completion
    texts = [
        'model governance',
        'hybrid deployment',
        'watsonx.ai is targeted',
        'enterprise ready',
        'use case',
        'use cases'
    ]
    ...
    review = review.lower()
    # Define a threshold for fuzzy matching. You can adjust this value based on your needs.
    threshold = 75
    
    # Count how many of the texts have a match (fuzzy) in the review irrespective of order
    #count = sum(1 for text in texts if fuzz.token_set_ratio(text, review.lower()) >= threshold)
    count = 0
    not_mentioned_datapoints = []
    for text in texts:
        if fuzz.token_set_ratio(text, review.lower()) >= threshold:
            count += 1
        else:
            not_mentioned_datapoints.append(text)
    
    if count >=5:
        count = 5
    
    # keeping only 'use cases'
    not_mentioned_datapoints = filter_similar_words(not_mentioned_datapoints)
    not_mentioned_datapoints_str = ', '.join(not_mentioned_datapoints)

    return {"count": count, "not_mentioned_datapoints_str": not_mentioned_datapoints_str}
...
def get_star_rating(rating):
    """Return the corresponding star emoji string based on the integer rating."""
    full_star = "⭐"
    empty_star = "☆"
